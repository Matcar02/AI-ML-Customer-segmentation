...
## Introduction
<h3> In the first part of the project, we deal with a customer database where customers' orders in Brazil are registered. There are many informations stored for each order, including the price spent and also some relevant information about the customer and sellers themselves; 
We then have done some data exploration to check for null values, duplicates, and useless features. 
...
This result can be seen in the following dataframe: <br/>
...
- For high and top spenders, the firm could as well promote the usual products they buy, but at the same time propose more general-user products to increase the already large Lifetime value they have.
